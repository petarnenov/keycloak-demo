# P1 ↔ keycloak-demo: Architecture and Workflows

Detailed architecture report for the full auth lifecycle between P1 (legacy Tomcat session) and keycloak-demo (OIDC + MFE) — login, logout, expired session, and all four supported entry methods.
...
| # | Method | Identifier | Credential source | Federated? | MFA |
|---|---|---|---|---|---|
| **A** | Local credentials | Username | user-service (P1 user store via Keycloak User Storage SPI) | No | Email OTP |
| **B** | Local credentials | Email | user-service (`loginWithEmailAllowed=true`) | No | Email OTP |
| **C** | OAuth federated — P1 | (SAML NameID) | P1 Tomcat session | Yes — SAML Identity Brokering | Inherited from P1 |
| **D** | OAuth federated — Social | (provider sub) | Google / Microsoft / GitHub | Yes — OIDC Identity Brokering | Inherited from provider |

**Default = A.** Email/password (B) is a fallback on the same flow. C is taken when the user reaches Keycloak with `kc_idp_hint=p1` (e.g., from the P1 sidebar). D is taken via a "Login with Google" button on the login screen.
...
### 1.3 Tokens, cookies, and session artifacts
...
### 2.1 Method A — Username + Password (default)
...
3. **Login screen**
   - Keycloak renders the `mfe-shell` theme login form (`keycloak/themes/mfe-shell/login/`).
   - Form fields:
     - **Username or email** (single input — `loginWithEmailAllowed=true` accepts both)
     - **Password**
     - "Login with P1" button (`kc_idp_hint=p1` link)
     - "Login with Google" / "Login with Microsoft" buttons (only if IdPs configured)
...
4. **Username/password submission**
   ...
   - Behind the scenes (`auth-username-password-form` authenticator):
     ...
     Keycloak                                user-service
        ...
        │  ◄────────────────────────────────────────── 200 {valid: true}
     ```
   - Fail-closed: any non-200 → false → "Invalid credentials".

5. **Email OTP step (`demo-email-otp` authenticator)**
   - **Trust cookie check first:**
     - HMAC-verified `KC_DEMO_OTP_TRUSTED` cookie present and not expired → `context.success()`, skip to step 7.
   - **No valid trust cookie:**
     - 6-digit `SecureRandom` code generated, stored in `AuthenticationSessionModel`.
     - `ResendClient.send()` POSTs the email (sandbox fallback: code logged at INFO).
     - User enters the code in the `email-otp.ftl` form.
...
### 2.2 Method B — Email + Password

Identical to A, except **step 4**: user enters email in the "Username or email" input. Keycloak's `auth-username-password-form` uses the `user-service` lookup with email criterion (`GET /users?email=...`); everything else is the same.

**Why no separate flow:** `loginWithEmailAllowed=true` in realm config makes a single input sufficient. Single-input UX > radio-style "Username vs Email" tab.

### 2.3 Method C — OAuth federated via P1 (SAML IdP Brokering)

This is the flow defined in `p1-sso-integration-research.md` Solution 2. It has **two sub-flows**:

#### C1 — IdP-initiated (from the P1 sidebar)

1. User is already logged into P1 (has JSESSIONID).
2. User clicks **"Demo MFE-BFF"** in the sidebar.
...
7. Keycloak validates signature (against the pre-configured P1 X509 cert), validates audience, time conditions.
8. **First Broker Login flow:**
   - Lookup user by NameID (email or UUID — TBD).
...
10. Steps 6–7 of Method A (token exchange + hydration).

#### C2 — SP-initiated (from the Keycloak login screen)

1. User landed on the login screen (e.g., opened `localhost:5173/` directly with no P1 context).
2. Clicks **"Login with P1"** button (rendered if `p1` IdP is configured).
...
5. P1 checks session:
   - **Active session:** silent — builds Response, returns auto-submit form (step 5 of C1).
...
### 2.4 Method D — OAuth federated via Social Provider (Google / Microsoft)

1. User on Keycloak login screen, clicks "Login with Google".
2. Browser → `GET /realms/demo-realm/broker/google/login?...`.
3. Keycloak builds an OAuth 2.0 authorization request, redirects to Google.
4. User completes Google login (potentially Google 2FA).
5. Google redirects back → `GET /realms/demo-realm/broker/google/endpoint?code=...&state=...`.
6. Keycloak exchanges the code for Google tokens, fetches Google userinfo.
7. **First Broker Login flow** (same as C1 step 8): auto-link by email or auto-create.
8. Keycloak mints its own session, redirects → `localhost:5173/?code=...`.
9. Steps 6–7 of Method A.

**Configuration:** Add Identity Provider → OIDC v1.0 → Google. Client ID/Secret from Google Cloud Console. Same pattern for Microsoft (Azure AD) or GitHub.

**Why not Google One Tap:** Requires the JS SDK on the shell page, complicates CSP, and adds nothing standard OIDC redirect doesn't. Skip.
...
4. Keycloak:
   ...
   - **NOTE:** `KC_DEMO_OTP_TRUSTED` cookie remains (by design — represents a trusted device, not session).
...
6. Shell catches the event: `keycloak.onAuthLogout` → `setAuthenticated(false)` → `queryClient.clear()` (clears every cached query → all "authenticated" UI rewinds in the same render tick).

### 3.2 Federated logout — Methods C, D

Additional steps between 4 and 5 of section 3.1:

4a. Keycloak inspects the user's session for active broker linkages (i.e., user came in through `p1` IdP).
...
4c. P1 `IdpSloAction`:
   ...
   - Destroys the P1 Tomcat session (mirrors `LogOutAction.java` behavior — clears `LOGGED_ADVISER`, `LOGGED_USER` keys).
   - Returns a signed `<samlp:LogoutResponse>` to Keycloak.
4d. Keycloak proceeds with local logout (step 5).

**For OIDC social providers (Google):** RP-initiated logout — Keycloak calls Google's `/revoke` endpoint OR `/end_session_endpoint`. Note: not all providers support this; if not, the Google session persists in the browser (this is OK — separate trust domain).
...
- Tab B's next `host.auth.getToken()` calls `keycloak.updateToken(30)` → POSTs `/token` with refresh token.
...
| OTP Trust Cookie | 60 min | 60 min or 0 (disabled) | `OTP_TRUST_WINDOW_MINUTES` env |
...
- If access token has ≤30s remaining → POST `/token` with refresh token:
  ...
  POST /realms/demo-realm/protocol/openid-connect/token
     ...
     client_id=mfe-shell-client
  ```
- Response: new `{ access_token, refresh_token (rotated if enabled), id_token, expires_in }`.
- keycloak-js replaces tokens in memory.

### 4.3 Refresh token expiry → forced re-auth

When the refresh token itself is expired (>30 min idle):

1. Shell calls `host.auth.getToken()` to make a fetch.
2. `keycloak.updateToken(30)` POSTs the refresh token.
...
5. `AuthProvider` catches:
   ...
   ```
6. Shell renders anonymous UI. User sees "Sign in" button.
7. User clicks → full login flow from section 2.
...
User reloads the tab. Shell process memory is gone (no tokens). But the realm cookie is still valid:
...
2. Hidden iframe loads:
   ...
   GET /realms/demo-realm/protocol/openid-connect/auth
      ...
      &code_challenge=...
   ```
3. Keycloak sees the valid `KEYCLOAK_IDENTITY` cookie → returns `302` with the auth code in the iframe URL.
4. keycloak-js parent reads the iframe URL via postMessage, exchanges code for tokens (PKCE).
5. `authenticated = true` without user interaction.
...
Even if the user is active, after 10h:
1. Keycloak invalidates the session.
2. Next refresh attempt → `400 invalid_grant`.
3. Same flow as section 4.3.

This protects against **forever sessions** in case of token leak (refresh token rotation would be invalidated when the realm session dies).

### 4.6 Idle timeout vs Max timeout

| Type | Trigger | Default | What it protects |
|---|---|---|---|
| **Idle** | No activity (no token refresh, no auth iframe ping) for X | 30 min | Forgotten tabs, public computers |
| **Max** | Absolute time since login | 10h | Compromised long-lived credentials |

Refresh-token use **resets the idle clock** but does not extend max. Max is enforced regardless of activity.
...
| Two tabs both refresh simultaneously | Both POST `/token` with same refresh token; rotation enabled → second one fails | Use `BroadcastChannel` to dedupe; or accept a brief re-auth flash in one tab |
| Clock skew between Keycloak and BFF | JWT `exp`/`nbf` validation fails sporadically | Both NTP-synced; allow `clockSkew=30s` in BFF JWT validator |
...
| Browser blocks third-party cookies | `check-sso` iframe fails (iframe cookies blocked) | Same-origin only (Keycloak on same domain as shell in prod). Or use `prompt=none` redirect (not iframe). |
...
### 5.1 Account linking on collision

Scenario: user already has a local Keycloak account (`demoadmin@example.com`). Same email attempts login through P1 (Method C). What does First Broker Login do?

**Configured behavior (recommended):**
- Auto-link by email (silent).
- New broker identity (P1) added to the existing Keycloak user record.
- User session contains both `realm_access.roles` (from local) and SAML attributes (from P1) — merge logic in claim mapper.

**Alternative — manual review:**
- Default Keycloak behavior: show "Account exists with same email — review identities" screen.
- User authenticates with the **existing local password** to confirm.
- Then link.
- Better security, worse UX. Choose per threat model.

---

## 6. Security and guards
...
| Token theft via XSS | Tokens **in memory only** (no localStorage). XSS still gives access during session window, but does not survive page reload. |
| CSRF on Bearer endpoints | Bearer auth is stateless — not vulnerable. (CSRF concern only if migrating to cookie-based BFF sessions.) |
...
| Federation SAML signature stripping | Keycloak SAML SP enforces signature on assertion AND/OR response. |
...
**Lax vs Strict:** Lax allows cross-site GET navigation (needed for the OIDC redirect dance). Strict breaks login flows from external links.
...
| Keycloak ↔ user-service | TLS in production (currently HTTP in compose) |
...
Drawn from the current keycloak-demo `LOGIN.md` "Must-fix before production" plus new items for P1 federation:
...
- [ ] Audience restriction in P1's SAML Response = Keycloak's broker EntityID
...
- [ ] Every SAML emission in P1 logs `SECURITY_EVENT: SAML_ISSUED user=<uuid> firm=<cd> target=<sp>`
...
- [ ] Loading states for silent SSO check (don't show "Sign in" for 200ms before silent SSO completes)
- [ ] Clear error messaging for federation failures ("Login with P1 unavailable — please try local credentials")

---

## 9. Files by component
...
See `p1-sso-integration-research.md` section 4.3 — full list of 5 new Java files + sidebar entry + Struts XML.

---

## 10. Open decision points

Carried forward from `p1-sso-integration-research.md` plus new ones from this architecture:
...
4. **Keycloak deployment topology** in production
5. **SLO scope** in MVP — front-channel SLO or skip

### New (from this architecture work)

6. **Account linking policy:** Auto-link by email (smooth UX) or Manual Review (stronger security)?
7. **Social IdPs:** include Google/Microsoft in MVP, or P1 only?
8. **Refresh token rotation:** enable at realm from the start, or phase 2?
9. **Multi-tab sync:** `BroadcastChannel` in MVP, or live-with-it?
10. **Audience scoping per MFE:** one shared audience (current) or token exchange per MFE call (production)?
11. **Email OTP for federated users:** **always skipped** (current design) or allowed as a second factor (e.g. `acr` claim "level2")?
...
| Parameter | Value |
|---|---|
| **Supported login methods** | A: username+password • B: email+password • C: SAML federation (P1) • D: OIDC social (Google/MS) |
| **MFA** | Email OTP for A/B (skipped for C/D — trust delegated) |
| **Logout** | Federated: Keycloak `/logout` + SAML SLO to P1; tokens cleared in-memory; QueryClient cleared |
...
| **Total effort for full architecture** | ~15 days (P1 IdP) + 2 days (Keycloak config) + 3 days (Shell hardening) + 5 days (production hardening) = **~25 days** |
