...
Two standalone non-MFE apps share `demo-realm` and the same P1 SAML federation:

- **demo-billing** (`frontend/apps/demo-billing/`, `https://billing.geowealth.int:5184`, OIDC client `demo-billing-client`). Plain white React app — "Under construction" page that proves the SSO wiring. P1-only auth via `kc_idp_hint=p1` (same pattern as the shell after Phase 13). Pairs with `bff-billing:8084` for JWT validation (no role gate — any authenticated user). Reached from P1's sidebar under **Integrations → Demo Billing**. Requires two setup steps: `/etc/hosts` entry `127.0.0.1 billing.geowealth.int` AND an mkcert-issued TLS certificate at `proxy/certs/billing.geowealth.int.{crt,key}` (Vite preview serves HTTPS — Keycloak.js v26 requires Web Crypto API which is gated behind secure contexts; `billing.geowealth.int` isn't a loopback hostname so the browser would otherwise refuse to expose `crypto.subtle`). Generate with `mkcert -cert-file proxy/certs/billing.geowealth.int.crt -key-file proxy/certs/billing.geowealth.int.key billing.geowealth.int localhost 127.0.0.1` (mkcert installs a local CA so the cert is trusted without browser warnings).
- **geowealth-poc** (`frontend/apps/geowealth-poc/`, `*.localhost:5174`). Whitelabel POC against `geowealth-realm` — entirely separate concern. See `whitelabel-relevance-report.md` for the relationship to demo-realm.
...
| BFF source | `podman compose up -d --build --force-recreate bff-client bff-ops bff-admin bff-billing` — one build, four recreates (they share the image). `--force-recreate` is the belt-and-suspenders bit: without it, some compose implementations don't notice the image ID moved and leave the old containers running. |
| `demo-billing` source | restart container: `podman compose up -d --force-recreate demo-billing` (same `build + vite preview` pattern as the MFEs). Requires `/etc/hosts` entry `127.0.0.1 billing.geowealth.int` on the host machine. |
