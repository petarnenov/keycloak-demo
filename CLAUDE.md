...
1. Entry point (either): user clicks a P1/Integrations sidebar link, OR the SPA's `keycloak.init({ onLoad: 'check-sso' })` finds no session and fires `keycloak.login({ idpHint: 'p1' })`. Both build the same URL: `http://localhost:8898/realms/demo-realm/protocol/openid-connect/auth?client_id=demo-<name>-client&response_type=code&scope=openid&redirect_uri=https://<name>.geowealth.int:518X/&kc_idp_hint=p1&state=…`.
