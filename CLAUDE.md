...
- `token-handler/` — the **extracted shared auth service** (industry-standard BFF/Token Handler as a separate runtime). It IS `bff-core` with no domain controller (`mainClass = demo.bff.core.Bff`), built into one **generic, env-driven image** (`SESSION_COOKIE_NAME`, `OAUTH_CLIENT_ID`, …). Compose runs one instance per domain (`token-handler-billing` = BSESSION/demo-billing-client, `token-handler-trading` = TSESSION/demo-trading-client). **It owns the active auth: login, callback, `/auth/me`, `/auth/logout`, `/backchannel-logout`, token refresh, Redis sessions.** nginx routes each domain's `/(oauth|auth)/` + `/logout` here; `/api/<name>` still goes to `bff-<name>`. The token-handler and its domain BFF **share the same Redis + session cookie + OIDC client**, so the session the token-handler writes on login is read by the BFF for `/api` authz. Net effect: a shared-auth fix → redeploy the token-handler image only; the domain data apps are untouched and users are not logged out (sessions live in Redis). Each KC client's `backchannel.logout.url` points at its `token-handler-<name>`. (Refresh runs in both the token-handler and the BFF — benign because the realm has `revokeRefreshToken=false`. Forward-auth / header-injected identity, which would make the BFF fully auth-unaware so EVERY auth fix is token-handler-only, is the documented next step — see `token-handler-plan.md`.)
...
- `docker-compose.yml` — services: `postgres`, `redis` (shared BFF/token-handler session + sid store, B2), `keycloak` (stock `quay.io/keycloak/keycloak:26.0.7`), and per domain: `demo-<name>` (web), `bff-<name>` (data) + `token-handler-<name>` (auth).
...
| `bff-core/` **auth-flow** source (login/callback/`/auth/*`/logout/backchannel/token-refresh) | **Token-handler only** — `docker compose build --no-cache token-handler-billing && docker compose up -d --force-recreate token-handler-billing token-handler-trading` (one image, two instances). nginx routes `/(oauth\|auth)/` + `/logout` there, so the fix is live; **`bff-billing`/`bff-trading` are NOT rebuilt and users are not logged out** (sessions in Redis). This is the whole point of the token-handler split. |
| `bff-core/` source that the **data BFF still runs** (`SubdomainRequirementFilter`, `Tier23Gate`, `AuthClaims`, session-read; and `TokenRefreshFilter` until forward-auth lands) | rebuild the data BFFs too — `docker compose build --no-cache bff-billing bff-trading && docker compose up -d --force-recreate bff-billing bff-trading`. |
