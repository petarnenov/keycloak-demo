...
- `token-handler/` — the **extracted shared auth service** (industry-standard BFF/Token Handler as a separate runtime). It IS `bff-core` with no domain controller (`mainClass = demo.bff.core.Bff`), built into one **generic, env-driven image** (`SESSION_COOKIE_NAME`, `OAUTH_CLIENT_ID`, …). Compose runs one instance per domain (`token-handler-billing` = BSESSION/demo-billing-client, `token-handler-trading` = TSESSION/demo-trading-client). **It owns ALL auth: login, callback, `/auth/me`, `/auth/logout`, `/backchannel-logout`, token refresh, Redis sessions, AND the per-request authorization decision (`GET /auth/verify`).** Forward-auth: nginx routes each domain's `/(oauth|auth)/` + `/logout` here, and for `/api/<name>` it runs an `auth_request` against the token-handler's `/auth/verify` (validate + refresh the session, run coarse + the subdomain Tier-2 gate) → on 200 it copies the returned `X-Auth-*` identity onto the upstream request and **drops the session cookie**, then proxies to `bff-<name>`. So **the domain BFFs are fully auth-unaware**: no session, no token refresh, no security filters — they read identity from the `X-Auth-*` headers (`HeaderIdentity`) and only serve data + the Tier-3 list `refine` (which filters the domain's own rows). Net effect: **every shared-auth fix → redeploy the token-handler image only**; the domain data apps are untouched and users are not logged out (sessions live in Redis). Each KC client's `backchannel.logout.url` points at its `token-handler-<name>`. The token-handler+BFF still share Redis + the session cookie + the OIDC client (so `/auth/verify` and the BFF agree on identity). See `token-handler-plan.md`.
...
| `bff-core/` **auth** source (login/callback/`/auth/*`/`/auth/verify`/logout/backchannel/token-refresh/session/`SubdomainAuthorizer`) | **Token-handler ONLY** — `docker compose build --no-cache token-handler-billing && docker compose up -d --force-recreate token-handler-billing token-handler-trading` (one image, two instances). Forward-auth: the data BFFs run NO auth, so the fix is live the moment the token-handlers restart; **`bff-billing`/`bff-trading` are NOT rebuilt and users are not logged out** (sessions in Redis). |
