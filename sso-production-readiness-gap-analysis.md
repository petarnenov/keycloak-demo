...
| **B5 (code core)** SAML keystore re-read per signature + `/tmp`-wipe fragility | `AbstractSamlAuthenticationResponseBuilder` re-read & re-parsed the PKCS#12 from disk on **every** signature (twice per Response); now an **mtime-aware cache** builds the `Credential` once and reloads only when the keystore file's last-modified time changes. Eliminates the per-request disk parse; preserves hot rotation (`sso-dev-keystore.sh` rewrites the file → mtime change → reload, no Tomcat restart); and makes signing resilient to the file vanishing at runtime. | `geowealth/.../AbstractSamlAuthenticationResponseBuilder.java` (`CREDENTIAL_CACHE`) | **Verified live:** after one login (cache populated), `/tmp/p1-idp-dev.p12` was **deleted** and a second login still emitted signed Responses (`SAML_ISSUED`, no "SAML emission failed") — before the fix this 500'd. Signing-path unit tests stay green. **Still ops (out of code scope):** the keystore *path* (move off `/tmp`), provisioning via a secret manager, and a distinct key/store password. |
...
| **B5 (ops remainder)** keystore path/secrets | The code core (per-request re-read, `/tmp`-wipe fragility) was fixed — moved to **Fixed and verified**. The keystore *location* (`/tmp`), secret-manager provisioning, and a distinct key/store password remain deployment concerns. | Move the keystore off `/tmp` onto a persistent secret-managed mount; distinct key vs store password. |
...
- **Session cookies correct.** Live trace: `kc_silent_attempt=1; Max-Age=120; Path=/; Secure;
  HttpOnly; SameSite=Lax`. The session cookie `GWSESSION` (one host-scoped name across all
  domains; replaced the per-domain `BSESSION`/`TSESSION`) is `httpOnly` (library-hardcoded),
