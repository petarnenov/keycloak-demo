...
7. **Phase G (object-instance scope) — ✅ DONE** (geowealth `564cdc6b26d` + keycloak-demo `5b6146b`). `POST /saml/idp/p1-authz-can.do` + `.../p1-authz-refine.do` (§2.9), both backed by `PolicyRuleManager.refineUUIDs` (one rule-set load + in-memory intersect; `can` = single-candidate refine). The BFFs `refineByObjectAccess` the list endpoints (`invoices`, `orders`) — the `loadCustomerViewableAccounts` pattern.
