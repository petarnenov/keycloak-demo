...
8. **`gw-superadmin` cross-firm override — ✅ DONE** (geowealth *SSO Phase 20* `7a7bcf0abf5` + keycloak-demo). The `gwAdmin || canX()` override is now wired on all three layers, since `gwAdminFlag` is a call-site convention that `PolicyRuleManager`/`refineUUIDs` do **not** apply themselves: (a) `realm-export.json` gains the `gw-superadmin` realm role + `roles-gw-superadmin-from-saml` mapper; (b) `SsoRoleTranslator` emits `gw-superadmin` from `gwAdminFlag` so the JWT carries it, and the P1 `/can` `/refine` short-circuit for gwAdmin server-side; (c) the BFFs skip the fine check / return the full list when the JWT has the `gw-superadmin` role. **Still outstanding** (deploy/data, not code): seeding real per-firm `roleCapabilityMappings`, mapping the demo's fictional objects to real P1 `ObjectType`/UUID ids, and a reachable P1 — all needed to flip `app.authz.fine-enabled` on end-to-end. Realm change is `IGNORE_EXISTING`, so on a running stack POST the role+mapper via admin API (or `./start.sh --reset`).
