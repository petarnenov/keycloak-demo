...
> **Architecture update (2026-06-15).** Part 2 below was written against a per-domain-BFF model where each domain's BFF held its own auth, its own `P1AuthzClient`, and its own `@Secured` controller gate. **That has since changed.** Auth and the Tier-1/Tier-2 P1 authorization client now run in **one multi-tenant `token-handler` service** (pure `bff-core`) that fronts every domain; its `GET /auth/verify` runs coarse + Tier-2 gating via nginx forward-auth. The per-domain data BFFs (`bff-billing`/`bff-trading`) are **auth-unaware** and run only the Tier-3 list `refine` next to the data. So when Part 2 says "the BFF" gates / caches authz, read it as "the token-handler" for Tier 1/2 and "the data BFF" for Tier 3. See `CLAUDE.md` (the `token-handler/` Layout entry) and `login-logout-algorithm.md` §7 for the shipped topology; the proposal's design *reasoning* (two-tier split, canonical capabilities, in-process cache, ≤60s TTL, fail-closed) is unchanged.
...
**→ Two firm-universal roles plus one global flag are the only role-like facts that mean the same thing across all firms.** That is the empirical ceiling on a fixed coarse vocabulary: the shipped `client`/`advisor`/`admin` triad is precisely this anchor set. `client` is the universal baseline (end-client identity, `entityTypeCd = CLIENT`); `advisor` is the **All Employees** / employee identity, set by the strict binary `isEmployee()` (`entityTypeCd = EMPLOYEE`) — an entity is exactly one type, never both client and employee; `admin` is the **Admins** role, i.e. a `(BACK_OFFICE, EXECUTE)` policy rule; `gwAdminFlag` → the open `gwAdmin` question in §2.7. Per-domain roles (`billing-admin`, `trading-trader`) are demo refinements layered on top, not P1 universals.
...
| `advisor` | `LoggedUser.advisor = user.isEmployee()` — a strict binary (`entityTypeCd == EMPLOYEE`); `false` for `CLIENT` | **identity type** (employee vs end-client), fixed at construction | `LoggedUser.java:49,247-248` → `User.java:363-364` → `UserData.java:1221-1222` (`USER_TYPE_EMPLOYEE=4`) |
...
The authz cache lives **in-process**, not in a shared Redis. As shipped this is the single multi-tenant `token-handler` (see the *Architecture update* note at the top) — Tier-2 gating and the `P1AuthzClient` now run there, not in per-domain BFFs. Reasons: (a) there is one token-handler fronting every domain, so a per-process cache is naturally shared across all tenants it serves; (b) a token-handler redeploy clears the slate naturally; (c) it keeps the authz cache off any new infrastructure dependency. Note this is specifically about the **authz cache**: the token-handler's *sessions* (and the sid registry) ARE Redis-backed (B2) so they survive a redeploy and users aren't logged out — the per-process authz cache is the deliberate exception, kept in-memory because it's cheap to rebuild and self-evicts on the ≤60s TTL.
...
| **Encode `gwAdminFlag` as a realm role.** | The doc already covered this: it's a *call-site* convention in P1, not an engine-level short-circuit. Surfacing it as a single global realm role (e.g. `gwAdmin`) IS the right move for tier 1, but `gwAdminFlag || hasX()` patterns still need to live in the BFF's authz layer. |
...
2. **Phase B (P1 PR — translation layer) — ✅ DONE** (geowealth *SSO Phase 18*, `241ed586214`). Implemented as a per-firm mapping in the firm's SSO config rather than a separate table — `FirmSSOConfig.roleCapabilityMappings : Map<roleName, [capability]>`, stored in the existing `FirmSSO.CONFIG` JSON, so onboarding adds a config entry with **no schema change**. The chain is `IdpSsoAction.deriveCapabilities` (coarse predicates from the loaded `LoggedUser`) → `SamlManager.deriveSsoCapabilities` (loads firm config + the user's roles under a session) → the pure, unit-tested `SsoRoleTranslator`. Earlier-sketched alternative for reference:
   ```sql
   -- (not used; the JSON-config approach above was chosen as "modelled on FirmSSOConfig")
   ...
   ```
   The SAML `roles` attribute now emits canonical strings, never firm-specific ones; `advisor` is emitted directly (the existing `roles-advisor-from-saml` mapper handles it). Per the **effective-not-membership** fix above, the coarse caps come from `isAdvisor()` / `canLoggedUserAccessBackOffice()`, and the translator falls back to coarse-only if the firm config or role lookup is unavailable so federation never 500s. The `roles-legacy-user-to-advisor-from-saml` Keycloak mapper can be removed once every firm has translation rows.
...
3. **Phase C (P1 PR — authz endpoint) — ✅ DONE** (geowealth *SSO Phase 19*, `564cdc6b26d`). `GET /saml/idp/p1-authz-me.do` (§2.2): `P1AuthzMeAction` extends `AbstractP1AuthzAction`, validates the bearer JWT via the Phase 17 `KeycloakJwksVerifier` (audience left open so any domain client's token is accepted), resolves the user from `sub`, and returns `firmCd` + `gwAdminFlag` + coarse `capabilities` + the fine `permissions` map from `PolicyRuleManager.loadCreateExecutePermissions`. (`p1Roles` diagnostic omitted in this pass — it needs an in-session role-name load; deferred as the only §2.2 gap.) One endpoint, every BFF consumes it.
4. **Phase D (BFFs) — ✅ DONE** (keycloak-demo `5b6146b`). `P1AuthzClient` in billing + trading: named `p1authz` http client, 2s read-timeout, per-`sub` in-process cache ≤60s (§2.3), forwards the user's own bearer token, fails closed on timeout/non-2xx. Tier-2 `requirePermission` on `summary`/`invoices` (billing) and `orders` (trading). **Opt-in** via `app.authz.fine-enabled` (default false) so the verified coarse-only demo is unchanged until P1's endpoints are reachable.
...
7. **Phase G (object-instance scope) — ✅ DONE** (geowealth `564cdc6b26d` + keycloak-demo `5b6146b`). `POST /saml/idp/p1-authz-can.do` + `.../p1-authz-refine.do` (§2.9), both backed by `PolicyRuleManager.refineUUIDs` (one rule-set load + in-memory intersect; `can` = single-candidate refine). The data BFFs (`bff-billing`/`bff-trading`) `refineByObjectAccess` the list endpoints (`invoices`, `orders`) — the `loadCustomerViewableAccounts` pattern. **As shipped, Tier-3 `refine` is the only authz the data BFFs run** (they are auth-unaware/forward-auth, next to the data); Tier-1 coarse gating and Tier-2 `requirePermission` now run upstream in the token-handler's `GET /auth/verify` (see the *Architecture update* note), not in the data BFFs.
8. **`gwAdmin` cross-firm override — ✅ DONE** (geowealth *SSO Phase 20* `7a7bcf0abf5` + keycloak-demo). The `gwAdmin || canX()` override is now wired on all three layers, since `gwAdminFlag` is a call-site convention that `PolicyRuleManager`/`refineUUIDs` do **not** apply themselves: (a) `realm-export.json` gains the `gwAdmin` realm role + `roles-gwAdmin-from-saml` mapper; (b) `SsoRoleTranslator` emits `gwAdmin` from `gwAdminFlag` so the JWT carries it, and the P1 `/can` `/refine` short-circuit for gwAdmin server-side; (c) the BFFs skip the fine check / return the full list when the JWT has the `gwAdmin` role. **Still outstanding** (deploy/data, not code): seeding real per-firm `roleCapabilityMappings`, mapping the demo's fictional objects to real P1 `ObjectType`/UUID ids, and a reachable P1 — all needed to flip `app.authz.fine-enabled` on end-to-end. Realm change is `IGNORE_EXISTING`, so on a running stack POST the role+mapper via admin API (or `./start.sh --reset`).
...
- Both validate the user's own JWT, fail **closed** on the 2s timeout (§2.7); `gwAdmin` short-circuits to "all allowed" without loading rules, mirroring `gwAdmin || canX(obj)`.
...
| `gwAdminFlag` call-site `gwAdmin \|\| canX()` (§1.8) | `gwAdmin` realm role + `isGwAdmin \|\| …` in BFF authz | all |
