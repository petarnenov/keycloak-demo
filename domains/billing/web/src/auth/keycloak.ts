...
/**
 * Keycloak is reachable on localhost:8898 even from billing.geowealth.int
 * because /etc/hosts only maps the billing hostname — localhost still
 * resolves to 127.0.0.1, and JSESSIONID-equivalent (Keycloak's KC_*
 * cookies) are scoped by Keycloak's own hostname, not the SPA's.
 */
