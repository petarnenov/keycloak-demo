 ...
 * http://changepath.localhost:8898). Phase 2's ThemeSelectorProvider reads
...
function keycloakBaseUrl(): string {
  if (typeof window === 'undefined') return 'http://localhost:8898';
  return `http://${window.location.hostname}:8898`;
}
