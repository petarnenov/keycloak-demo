...
**Date:** 2026-06-26 (unchanged for v2)
**Status:** Frozen for Phase 1 (user-service standalone). Still
authoritative in v2 — `user-service`'s Oracle reads have not changed
since the original schema freeze.
**Source of truth:** P1's Hibernate `.hbm.xml` files in
`nodejs/geowealth/src/main/java/com/geowealth/util/hibernate/`

This document fixes the exact Oracle column shape user-service depends
on. Columns the live schema actually uses; **column names ≠ what the
plan's prose suggested**. Authoritative names from the hbm files.
...
this with a recursive CTE (see §SQL below).
...
`com.gw.userservice.security.SHAPassword`. Do NOT reimplement from
scratch — the salt handling and label-prefix subtlety is non-obvious.

This algorithm is **legacy** — a SHA1 + optional salt is not appropriate
for new password storage. The bcrypt rehash-on-login migration is listed
as a follow-up MD (see
[`08-questions-and-answers.md`](08-questions-and-answers.md) §D6) and
will leave this contract intact (just adds a second verifier path).
...
Result formatted as `String[]` of `"<firmCd>:<ldapUid>"` to match the
shape P1's `PersonRegistry#resolveMemberships` emits.
...
1. Generate a 6-digit numeric token (cryptographic RNG).
2. Compute `SHA1(token)`.
3. `UPDATE ENTITY_TBL SET MFA_TOKEN = ?, MFA_TOKEN_EXPIRATION_DATE = ? WHERE ENTITY_ID = ?`
   with the SHA1 and `now + 5 minutes`.
4. Resolve the user's email (today: placeholder via `findEmail`; a
   future change walks `EMAIL_TBL` if/when one exists).
5. Return `{ tokenSentTo: "<email>", ttlSeconds: 300 }`.

`verifyToken(entityId, submitted)`:

1. `SELECT MFA_TOKEN, MFA_TOKEN_EXPIRATION_DATE FROM ENTITY_TBL WHERE ENTITY_ID = ?`.
2. Reject if `MFA_TOKEN_EXPIRATION_DATE < now` (expired).
3. Compare `SHA1(submitted)` with the stored hash via
   `MessageDigest.isEqual` (constant-time).
4. Return `{ valid: true|false }`.

The `EmailOtpAuthenticator` in Keycloak issues the email on the KC side
(via KC's `EmailSenderProvider` against KC's SMTP relay) — user-service
does not send mail itself, it just generates and stores the token.

## Cache policy

KC User Storage SPI caches user lookups (`EVICT_DAILY` by default; can
be tuned via the realm admin UI on the user-federation component).
Credential validation is NEVER cached — Keycloak honours this contract,
so a password change at `user-service` is effective on the next login
regardless of cache state. Roles are part of the user attributes blob
and are cache-stale until daily eviction — acceptable for the demo
because role changes are rare.

## Why this doc exists

The plan's prose used aspirational column names (`LDAP_PASSWORD_HASH`,
`PASSWORD_EXPIRATION_DATE`, `LOGIN_INACTIVATED_REASON_CD`, `ROLE_ID/UUID`).
The live schema uses abbreviations and one legacy typo
(`LOGIN_INACTIVED_*`). user-service code must match the live schema,
not the plan's prose. Future schema changes update this doc; the plan
stays the design.
